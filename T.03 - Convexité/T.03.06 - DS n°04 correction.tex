...
\rhead{\textcolor{red}{Correction $-$ DS n$^{\circ}$04 $-$ Convexité $-$ 20/11/2025}}
			...
			\begin{minipage}[]{15cm}
				\begin{center}
					\LARGE{\rule{0.5cm}{0pt}\rule[-0.25cm]{0pt}{0.9cm}
						\textcolor{red}{Correction $-$ DS n$^{\circ}$04\\Convexité}
					}
				\end{center}
			\end{minipage}
		}
...
%	\includegraphics[scale=0.2]{2025_11_06_3b44234b8b55fbc3078dg-1(1)}
	...
	\begin{enumMet}
		...
		\begin{corrBox}
			La fonction est concave sur $[-6 ; 1]$ puis convexe sur $[1 ; 6]$.
		\end{corrBox}
		\item Déterminer les éventuels points d'inflexion de $C_{f}$.
		\begin{corrBox}
			Le point d'inflexion a pour coordonnées $(1\ ;\ 1,5)$		
		\end{corrBox}
	\end{enumMet}
...
%	\begin{multicols}{2}
		\begin{enumMet}
			...
			\begin{corrBox}
				...
				$f''(x)=36x^2-12x-8=4(9x^2-3x-2)$
				
				$\Delta=9+4\times 9\times 2=81$
				
				$x_1=-\dfrac{1}{3}$ et $x_2=\dfrac{2}{3}$
				
				\begin{tikzpicture}
					\tkzTabInit{$x$ / 1 , $f''(x)$ / 1}{$-\infty$, $-\dfrac{1}{3}$,$\dfrac{2}{3}$, $+\infty$}
					\tkzTabLine{, +, z, -, z,+,}
				\end{tikzpicture}
			\end{corrBox}
			...
			\begin{corrBox}
				\begin{tikzpicture}
					\tkzTabInit{$x$ / 1 , $f(x)$ / 1}{$-\infty$, $-\dfrac{1}{3}$,$\dfrac{2}{3}$, $+\infty$}
					\tkzTabLine{, \text{convexe}, z, \text{concave}, z,\text{convexe},}
				\end{tikzpicture}
			\end{corrBox}
			\item Donner les coordonnées du ou des points d'inflexion
			\begin{corrBox}
				$f\left(-\dfrac{1}{3}\right)= -\dfrac{1}{3}$ et $f\left(\dfrac{2}{3}\right)= -\dfrac{16}{9}$.
				
				Les 2 points d'inflexion ont comme coordonnées $\left(-\dfrac{1}{3}\ ;\ -\dfrac{1}{3}\right)$ et $\left(\dfrac{2}{3}\ ;\ -\dfrac{16}{9}\right)$
			\end{corrBox}
		\end{enumMet}
%	\end{multicols}
...
%	\begin{multicols}{2}
		\begin{enumMet}
			...
			\begin{corrBox}
				...
				$f''(x)=\dfrac{10\times (8x+24)}{(2x+6)^4}$
				
				\begin{tikzpicture}
					\tkzTab{$x$ / 1 , $f''(x)$ / 1, $f'(x)$ / 1.5}{$-\infty$, $-3$, $+\infty$}
					{, -, d, +, }
					{+/ , -/, +/}
				\end{tikzpicture}		
			\end{corrBox}
			...
			\begin{corrBox}
				La fonction est concave sur $[-\infty ; -3]$ puis convexe sur $[-3 ; +\infty]$.
			\end{corrBox}
		\end{enumMet}
%	\end{multicols}
	...
	\begin{enumMet}
		...
		\end{center}
		\begin{corrBox}
			La fonction $f$ est fonction polynome de degré 2 avec $a>0$. Sa courbe représentative est donc une parabole \guillemets{tournée vers le haut}. Il s'agit donc de la courbe rouge.		
			
			La fonction $g$ est fonction polynome de degré 2 avec $a<0$. Sa courbe représentative est donc une parabole \guillemets{tournée vers le bas}. Il s'agit donc de la courbe bleue.
			
			$f$ semble donc convexe et $g$ concave.		
		\end{corrBox}
		\item \begin{enumMet}
			...
			\begin{corrBox}
				$f''(x)>0$ pour tout $x \in \R$ donc $f'$ est croissante sur \R donc $f$ est convexe sur \R. 		
				
				$g''(x)<0$ pour tout $x \in \R$ donc $g'$ est décroissante sur \R donc $g$ est concave sur \R. 		
			\end{corrBox}
		\end{enumMet}
...
%	\begin{multicols}{2}
		\begin{enumMet}
			...
			\begin{corrBox}
				$f'(x)=-8x^3-6x+5$ et $f''(x)=-24x^2-6<0$ pour tout $x \in \R$.
				
				$f$ est donc concave sur \R.		
			\end{corrBox}
			\item $f(x)=2 \mathrm{e}^{-3 x}+4x^2$
			\begin{corrBox}
				$f'(x)=2\times (-3)e^{-3x}+8x$ et $f''(x)=-6\times (-3)e^{-3x}+8=18e^{-3x}+8>0$ pour tout $x \in \R$.
